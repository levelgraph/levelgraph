...
module.exports = function levelgraph(leveldb, options, IDBStoreReadyCallback) {
  ...
  if (typeof leveldb === 'string') {
    ...
    if(IDBStoreReadyCallback){
      var wrapped_cb = function(err, leveldb){
        var db = initDB(options, leveldb);
        IDBStoreReadyCallback(err, db);
      };
    }
    leveldb = levelup(name, options, wrapped_cb);
  }

  return initDB(options,leveldb);
};
