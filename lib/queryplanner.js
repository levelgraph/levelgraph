...
doSortQueryPlan = function(first, second) {
  ...
  var firstQueryMask = utilities.queryMask(first)
    ...
    , firstIndexes = (first.index) ? [first.index] : utilities.possibleIndexes(Object.keys(firstQueryMask))
  ...
  //console.log("sameVariablesKeys", sameVariablesKeys);

  if (!sameVariablesKeys && commonIndexes.length > 1) {
    second.index = commonIndexes[1];
    second.stream = SortJoinStream;
    return second;
  } else if (commonIndexes.length > 0) {
